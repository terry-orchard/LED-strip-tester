...
camera.framerate = 12
# camera.exposure_mode = 'spotlight' # one way to change exposure
camera.exposure_compensation = -22 # one way to change exposure
raw_capture = PiRGBArray(camera, size=(640, 480))
time.sleep(0.1) # warmup camera sensor

# my variables for keeping track of stuff:
looking = 1 # (flag) found the lights yet?
    ...
    if(count<200): # for the first 200 frames, build up a list of lights
        for c in contours:
            ...
            if(w*h < 20): # if the shape is small
                if(len(found) == 0): found.append(center) # check if we have any points yet
                    ...
                    for p in found: # go through points we already have
                        if (p[0]+1 >= x and p[0]-1 <= x+w and p[1]+2 >= y and p[1]-2 <= y+h): stored = 1 # if so, don't store it
                    if not(stored): found.append(center) # if not, store it
        ...
        for p in found: # mark all the found points
            ...
            if(avg_lit[1] >= 200):
                cv2.putText(cropped, repr(int(avg_lit[1])), (p[0]+5,p[1]), cv2.FONT_HERSHEY_PLAIN, .5, (0,0,avg_lit[1])) #red
                numlit+=1
            else: cv2.putText(cropped, repr(int(avg_lit[1])), (p[0]+5,p[1]), cv2.FONT_HERSHEY_PLAIN, .5, (150,avg_lit[1],0)) # blue
        ...
        if(float( float(cv2.countNonZero(lit))/float(4) ) > 0.50): # if more light than dark, it's ON
            ...
            if(led_r >= led_g and led_r >= led_b): cv2.putText(cropped, 'RED', (rgb_led[0]+25,rgb_led[1]), cv2.FONT_HERSHEY_PLAIN, 1, (50,50,250))
            elif(led_g >= led_r and led_g >= led_b): cv2.putText(cropped, 'GREEN', (rgb_led[0]+25,rgb_led[1]), cv2.FONT_HERSHEY_PLAIN, 1, (50,250,50))
            else: cv2.putText(cropped, 'BLUE', (rgb_led[0]+25,rgb_led[1]), cv2.FONT_HERSHEY_PLAIN, 1, (250,50,50))
        else: cv2.putText(cropped, 'OFF', (rgb_led[0]+25,rgb_led[1]), cv2.FONT_HERSHEY_PLAIN, 1, (200,200,200)) # not enough light, must be OFF

        cv2.putText(cropped, '#'+repr(numlit), (5,270), cv2.FONT_HERSHEY_PLAIN, 1, (20,10,250))
#        if(numlit == 0): cv2.putText(cropped, "ZERO NOW", (1,10), cv2.FONT_HERSHEY_PLAIN, 1, (0,0,250))
