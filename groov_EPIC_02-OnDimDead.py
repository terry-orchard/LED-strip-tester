...
#        sublist.pop(0) '''
    ...
    if key == ord("r"):
        ...
        state = ['dead', 'on  ', 'dim ']
        print("Board status before checking for dim lights:")
        for led in status:
            print(("# " + repr(led[0]) + "\t" + state[led[1]] + " @ " + repr(led[2])))
        temp = list(found) # temporary list of lights, so we don't mess up the original
        temp.pop(0) # don't check the rgb led
        ...
        print("Board status after checking for dim lights:")
        for led in status:
            print(("# " + repr(led[0]) + "\t" + state[led[1]] + " @ " + repr(led[2])))
